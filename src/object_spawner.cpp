...
ObjectSpawner::ObjectSpawner(ros::NodeHandle *node_handle)
    : tf_listener_(this->tfBuffer_), object_name("trash") {
  // object_name = "trash";
  is_spawned = false;
  ...
  ros::topic::waitForMessage<geometry_msgs::PoseWithCovarianceStamped>(
      "/explorer/amcl_pose", ros::Duration(10));

  nh_ = node_handle;
  pose_pub_ =
      nh_->advertise<gazebo_msgs::ModelState>("/gazebo/set_model_state", 10);
...
bool ObjectSpawner::spawn_object() {
  ...
  srv.request.initial_pose.position.x =
      (rand_r(&seed) % (std::abs(map_range[2] - map_range[0])) + map_range[0]);
  srv.request.initial_pose.position.y =
      (rand_r(&seed) % (std::abs(map_range[3] - map_range[1])) + map_range[1]);
  ...
  if (srv.response.success) {
    ...
    object_pose_tf_timer_ = nh_->createTimer(
        ros::Duration(0.1), &ObjectSpawner::publish_pose, this);
    // Start service server to listen if object is in hand
    // update_state_service_ = nh_->advertiseService(
    //     "/setObjectState", &ObjectSpawner::set_object_state_cb, this);
  } else {
