...
/**
 * @brief Function to be used for detecting the object to be collected. This will give out true if the object is detected while printing the x and y location for the object. otherwise it will return false
 * 
 * @return true 
 * @return false 
 */
bool DetectObject::detect_object() {
    ...
    if (contours_.size() > 0) {
        ...
        if (is_object_detected == false) {
            ...
            ROS_INFO_STREAM("[ObjectDetector] Object Detected!! Its location in image is at X: "
                                                    << x << ",\ty: " << y);
        }
    }
